var playersScore, activePlayer, lastCity, usedCities, tries;
...
function init() {
    ...
    document.getElementById('city-score-0').textContent = 0;
    document.getElementById('city-score-1').textContent = 0;
    document.getElementById('city-used-0').innerHTML = '';
    document.getElementById('city-used-1').innerHTML = '';
    document.querySelector('.city-player-0').classList.remove('current');
    document.querySelector('.city-player-1').classList.remove('current');
    document.querySelector('.city-player-0').classList.add('current');
}

function incorrectActionMessage(message, tries) {
    document.getElementById('city-input').value = "";
    document.getElementById('city-input').placeholder = message + ". Ещё " + (3 - tries) + " попытки";
}
...
function confirmWord () {
    ...
    if(city.length > 0 && index !== -1) {
        var isUsed = usedCities.indexOf(city) !== -1;
        if(!isUsed) {
            var lastSymbol = lastCity[lastCity.length - 1];
            switch(lastSymbol) {
                case 'ь': lastSymbol = lastCity[lastCity.length - 2]; break;
                case 'й': lastSymbol = 'и'; break;
                case 'ё': lastSymbol = 'е'; break;
            }

            if(lastCity.length !== 0 && lastSymbol !== city[0]) {
                tries++;
                incorrectActionMessage('Неподходящий город', tries);
            ...
            playersScore[activePlayer] += city.length;
            usedCities.push(city);
            
            var newCity = document.createElement('p');
            newCity.textContent = city;
            document.getElementById('city-used-' + activePlayer).appendChild(newCity);
            document.getElementById('city-score-' + activePlayer).textContent = playersScore[activePlayer];
            document.getElementById('city-input').value = "";

            lastCity = city;
        } else {
            tries++;
            incorrectActionMessage('Использованный город', tries);
            return;
        }
    } else if (tries < 2) {
        tries++;
        incorrectActionMessage('Несуществующий город', tries);
        return;
    } else {
        document.getElementById('city-input').value = "";
    }
...
function nextPlayer() {
    document.getElementById('city-input').placeholder = "Введите название города";
    document.querySelector('.city-player-' + activePlayer).classList.remove('current');
    tries = 0;
    activePlayer = activePlayer === 0 ? 1 : 0;
    document.querySelector('.city-player-' + activePlayer).classList.add('current');
}

document.querySelector('.city-btn-new').addEventListener('click', init);

document.querySelector('.city-btn').addEventListener('click', confirmWord);
